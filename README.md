...
Both arms read two DuckDB settings from the environment. `SIAB_DUCKDB_MEMORY_LIMIT` caps how much memory DuckDB may hold, in DuckDB's notation such as `8GB`, and `SIAB_DUCKDB_TEMP_DIR` names the folder it spills to beyond that. Unset, DuckDB takes 80 percent of the machine. Set both whenever a run may need more memory than the machine has. A run that reaches the default limit stops with `Out of Memory Error: failed to pin block`; with a limit and a spill folder set, DuckDB spills instead. The R arm at ten copies of the test delivery finished under a 4 GB limit at 10 percent more wall clock time.

The memory limit changes the imputed wages slightly. A seeded run reproduces exactly as long as the memory limit stays the same. Under a different limit, DuckDB adds up the leave-one-out means in a different order when it spills, so they differ in the last digits, and the random draw then lands differently for some episodes. The result is as valid as a run with a different seed: the draws come from the same model, and only the values for individual episodes differ. In the Python arm at eight copies of the test delivery, a 1 GB limit changed `wage_imp` on 237,304 rows against an uncapped run, the largest by 1,670 EUR a day. The R arm computes the same means in DuckDB and has not been measured this way. To reproduce a run value for value, keep the seed and the memory limit, and record both with the results.
