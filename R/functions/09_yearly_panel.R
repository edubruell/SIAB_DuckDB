...
build_yearly_panel <- function(connection, 
  ...
  tbl(connection, "data") |>
      ...
      mutate(cu_date  = as.Date(paste0(as.integer(year),md_string)),
             ...
             #missing tage_erw: 966 rows of the test data, all quelle 1 with no
             #erwstat. The coalesce puts the Stata reading back.
             tage_erw = if_else(quelle==1 & !coalesce(erwstat %in% c(102L, 121L, 122L, 141L, 144L), FALSE),tage_erw - overhang,tage_erw),
             #16_yearly_panel.do also allows the legacy code quelle == 16, which
             #does not occur in SIAB 7523 v2
