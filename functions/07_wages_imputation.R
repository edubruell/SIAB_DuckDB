...
impute_wages <- function(connection, log_file = NULL){
  ...
  imputation_plan <- tbl(connection, "data") |>
    ...
    collect() |>
    #pwalk matches plan columns to imputw arguments by name, and the names must
    #differ from the data columns they filter on
  ...
  imputw <- function(plan_year, plan_educ_tmp, plan_east){
    glue("Starting imputation run for year={plan_year}, educ= {plan_educ_tmp} and east = {plan_east}") |>
      log_info(namespace = "impute_wages")
